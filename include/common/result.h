...
struct Storage {
    ...
    template<typename U>
    const U& get(std::true_type) const {
    ...
    template<typename U>
    U get(std::false_type) {
        return std::move(*reinterpret_cast<U*>(&storage_));
    }

    void destroy(ok_tag) {
        if (initialized_) {
            get<T>(std::is_copy_constructible<T>()).~T();
            initialized_ = false;
        }
    }
    ...
    void destroy(err_tag) {
        if (initialized_) {
            get<E>(std::is_copy_constructible<E>()).~E();
            initialized_ = false;
        }
    }
...
struct Result {
    ...
    unwrap() const {
        if (isOk()) {
            return const_cast<storage_type&>(storage()).template get<U>(std::is_copy_constructible<U>());
        }
    ...
    E unwrapErr() const {
        if (isErr()) {
            return storage().template get<E>(std::is_copy_constructible<E>());
        }
...
private:
    ...
    T expect_impl(std::false_type) const { return const_cast<storage_type&>(storage_).template get<T>(std::is_copy_constructible<T>()); }
...
bool operator==(const Result<T, E>& lhs, const Result<T, E>& rhs) {
    static_assert(concept_n::EqualityComparable<T>::value, "T must be EqualityComparable for Result to be comparable");
    static_assert(concept_n::EqualityComparable<E>::value, "E must be EqualityComparable for Result to be comparable");
